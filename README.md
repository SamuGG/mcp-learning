...
The [MCP client](./mcp-client/README.md) is a Typescript console application using Ollama for chatting with the LLM model of your choice.
...
Calling tools is a manual process that must be implemented by observing the response object when `chatResponse.message.tool_calls.length > 0`.
